...
interface Props {
  ...
  // AI / callsheet props (passed through to AITools in right panel)
...
export default function ScriptDisplay({
  scenes, characters, title, project,
  onUpdateCharacter, onMergeCharacters, onUpdateLocation,
  onUpdateScene, onUpdateSceneById,
  onAddTeamMember, onUpdateTeamMember, onRemoveTeamMember,
  onGenerateCallsheet, generatingCallsheet, callsheetUrl, callsheetError,
}: Props) {
  ...
  return (
    <div className="flex h-full min-h-0 gap-4">
          ...
          >
            <div className="flex items-center justify-between gap-1">
              <div className="flex gap-1">
                <span className="rounded bg-gray-200 px-1 text-[10px] font-bold text-gray-600">{s.setting}</span>
                {s.time && <span className="rounded bg-gray-100 px-1 text-[10px] text-gray-500">{s.time}</span>}
              </div>
            ...
            {s.characters.length > 0 && (
              <div className="mt-1 flex flex-wrap gap-0.5">
                {s.characters.slice(0, 3).map((c) => (
                  <span key={c} className={`rounded px-1 text-[9px] font-bold ${colorMap[c] ?? "bg-gray-100 text-gray-600"}`}>
                    {c.split(" ")[0]}
                  </span>
                ))}
              ...
              {scene.parts.map((part, i) => {
                if (part.type === "dialogue") {
                  ...
                  return (
                    <div key={i} className="flex flex-col items-center text-center px-8">
                      <span className={`mb-1 rounded px-2 py-0.5 text-xs font-bold ${color}`}>{part.character}</span>
                ...
                return (
                  <p key={i} className="text-gray-600">
                    {segments.map((seg, j) =>
                      seg.isCharacter ? (
                        <span key={j} className={`rounded px-1 font-bold ${colorMap[seg.character!] ?? "bg-gray-100 text-gray-700"}`}>{seg.text}</span>
                      ) : (
      ...
      {/* RIGHT — Scene info / AI tools */}
      <aside className="hidden w-64 flex-shrink-0 flex-col rounded-xl border border-gray-200 bg-white shadow-sm sm:flex">
        {/* Panel header with toggle */}
        <div className="flex-shrink-0 flex items-center gap-1 border-b border-gray-100 px-3 py-2">
          <button
            onClick={() => setShowAI(false)}
            className={`flex-1 rounded-lg py-1 text-[11px] font-medium transition-colors ${
              !showAI ? "bg-gray-100 text-gray-800" : "text-gray-400 hover:text-gray-600"
            }`}
          >scene</button>
          <button
            onClick={() => setShowAI(true)}
            className={`flex-1 rounded-lg py-1 text-[11px] font-medium transition-colors flex items-center justify-center gap-1 ${
              showAI ? "bg-gray-100 text-gray-800" : "text-gray-400 hover:text-gray-600"
            }`}
          ><span className="text-[10px]">✶</span> ai</button>
        </div>
        ...
        <div className="flex-1 min-h-0 overflow-y-auto p-3">
          {showAI ? (
            project ? (
              <AITools
                project={project}
                onGenerateCallsheet={onGenerateCallsheet ?? (async () => {})}
                generatingCallsheet={generatingCallsheet ?? false}
                callsheetUrl={callsheetUrl}
                callsheetError={callsheetError}
              />
            ) : (
              <div className="text-xs text-gray-400 text-center mt-8">Load a script to use AI tools</div>
            )
          ) : (
